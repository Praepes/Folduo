...
public final class PairingConnectionCtx implements Closeable {
        ...
        for (; ; ) {
            switch (mState) {
                case ExchangingMsgs:
                    if (!doExchangeMsgs()) {
                        notifyResult();
                        throw new IOException("Exchanging message wasn't successful. " + mLastError);
                    }
                ...
                case ExchangingPeerInfo:
                    if (!doExchangePeerInfo()) {
                        notifyResult();
                        throw new IOException("Could not exchange peer info. " + mLastError);
                    }
                    notifyResult();
                    return;
                case Ready:
    ...
    private byte[] exportKeyingMaterial(SSLSocket sslSocket, int length) throws SSLException {
        // 反射 Conscrypt（hidden API，getDeclaredMethod + setAccessible 尝试绕过）
        try {
            ...
            // 先尝试 public 方法，再尝试 declared（hidden）
            Method exportKeyingMaterial = null;
            try {
                exportKeyingMaterial = conscryptClass.getMethod("exportKeyingMaterial", SSLSocket.class,
                        String.class, byte[].class, int.class);
            } catch (NoSuchMethodException ignored) {
                exportKeyingMaterial = conscryptClass.getDeclaredMethod("exportKeyingMaterial", SSLSocket.class,
                        String.class, byte[].class, int.class);
                exportKeyingMaterial.setAccessible(true);
            }
    ...
    private boolean doExchangeMsgs() throws IOException {
        ...
        if (theirHeader == null || !checkHeaderType(PairingPacketHeader.SPAKE2_MSG, theirHeader.type)) {
            mLastError = "SPAKE2 消息头无效或对端关闭";
            return false;
        }
        ...
        try {
            boolean ok = mPairingAuthCtx.initCipher(theirMsg);
            if (!ok) {
                mLastError = "SPAKE2 密码验证失败（配对码可能过期或不正确）";
            }
            return ok;
        } catch (Exception e) {
            mLastError = "SPAKE2 initCipher 异常: " + e;
    ...
    private boolean doExchangePeerInfo() throws IOException {
        ...
        PairingPacketHeader theirHeader = readHeader();
        if (theirHeader == null || !checkHeaderType(PairingPacketHeader.PEER_INFO, theirHeader.type)) {
            mLastError = "对端未响应 PeerInfo 或已断开（可能密码不一致）";
            return false;
        }
        ...
        if (decryptedMsg == null) {
            mLastError = "解密对端 PeerInfo 失败（SPAKE2 密钥不一致，配对码或 TLS 参数不符）";
        ...
        if (decryptedMsg.length != PeerInfo.MAX_PEER_INFO_SIZE) {
            mLastError = "对端 PeerInfo 长度异常 size=" + decryptedMsg.length;
